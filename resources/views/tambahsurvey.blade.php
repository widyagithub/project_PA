        ...
        <div class="row justify-content-center" >
         <div class="col-8" style="width: 80%;">
            <div class="card" style=" margin-left:200px; ">
                <div class="card-body">
                   <form action="/insertsurvey/{{$data->id}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <h3 class="text-center mb-4">TAMBAH DATA SURVEY</h3>
                    <div class="mb-3">
                    ...
                    <table width="100%">
                        <tr>
                                        ...
                                        <select class="form-control" name="sub_kriteria[<?php echo $valuex->nama_kriteria;?>]" style="margin-bottom:5px">
                                            <?php foreach ($datasubkriteria as $key => $value) {  
                                                if($valuex->id==$value->id_kriteria){ ?>
                                                    ...
                                                    <option value="<?php echo $value->nama_subkriteria."-".$value->bobot_subkriteria."-".$value->nilai_sub_kriteria."-".$valuex->bobot_kriteria; 
                                                    ?>"><?php echo $value->nama_subkriteria;?> </option>
                                                <?php } } ?>
                                            </select>
                                        <?php } ?>
                                    </td>
                            ...
                            <a href="/warga" class="btn btn-danger">Batal</a><br><br><br>
                        </form>
                    </div>
                </div>
            </div>
